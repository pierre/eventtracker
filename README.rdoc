...
The eventtracker library is a Java library to send events to the Collector (http://pierre.github.com/collector).
...
Each event accepted via the offerEvent(Event) API are directly serialized to disk using the com.ning:metrics.serialization-writer library for persistency. Events are then de-serialized and sent to the collector periodically and/or after a certain number of events have been offered.

The library supports all Collector API: HTTP based and Scribe (Thrift).
